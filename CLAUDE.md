...
| `RICK_REVIEW_BACKENDS` | Comma-separated rotation for review-phase handlers (default `codex,opencode,claude`). Set to a subset to limit to installed CLIs, or one name to disable rotation. `gemini` and `antigravity` are supported but must be opted in explicitly here. |
