...
| `pr-review` | pr-workspace → pr-jira-context → (14 pr-category reviewers ∥) → pr-consolidator → pr-cleanup | `dag=pr-review, source=gh:owner/repo#N` |
...
| `plan-btu` | confluence-reader → codebase-researcher → plan-architect → ⏸hint → estimator → ⏸hint → confluence-writer | `rick_run_workflow dag=plan-btu, page_id=<id>` |
...
The `pr-review` category reviewers are: `pr-security`, `pr-concurrency`, `pr-error-handling`, `pr-observability`, `pr-api-contract`, `pr-idempotency`, `pr-testing`, `pr-integration`, `pr-performance`, `pr-data`, `pr-hygiene`, `pr-vendor-resilience`, `pr-docs-concordance`, `pr-correctness`. Each has a narrow `## Your Domain (ONLY these)` scope in `internal/persona/prompts/pr-*.md` plus explicit boundary rules against the others. `pr-vendor-resilience` is polyglot (Go / JS-TS / PHP / network vendors) and self-scopes from the diff. `pr-docs-concordance` catches documentation/comment drift the diff introduces (a doc-comment that now lies about the changed code); it anchors findings on changed lines, so it does **not** catch stale references in files the PR doesn't touch — that cross-file gap is closed by the `pr-stale-reference` sweep (below). `pr-correctness` owns general **behavioral correctness** — logic bugs, broken invariants, regressions, and unreachable intended paths introduced by the diff — the "is this just plain wrong / does it silently break existing behavior" lens the other twelve leave uncovered; it defers every specialized failure mode to its sibling (a race → `pr-concurrency`, a swallowed error → `pr-error-handling`, etc.) and reports genuinely-ambiguous-but-unprovable behavior only as an advisory `minor` (never blocking).
...
`internal/mcp/` exposes 18 tools over JSON-RPC 2.0 (stdio/HTTP). The surface is aggressively consolidated — facade tools multiplex over fine-grained handlers (which remain the single source of truth) to keep the LLM's tool-selection context small. The 18: workflow (`rick_run_workflow` — also launches plan-btu via `dag=plan-btu`+`page_id`; `rick_workflow_inspect`; `rick_workflow_control`), jobs (`rick_consult`, `rick_run`, `rick_job_inspect`, `rick_job_cancel`), `rick_workspace`, `rick_wave_manager`, `rick_diff_viewer`, jira (`rick_jira_read`/`rick_jira_write`/`rick_jira_search`/`rick_jira_create`/`rick_jira_manage_links`), `rick_confluence`, `rick_create_pr`, `rick_project_sync`. Key multiplexers: `rick_workflow_inspect` reads per-workflow panels (status/timeline/tokens/verdicts/output/persona_output) with a `workflow_id`, or global panels (list/events/dead_letters; `list` filters by ticket/source/repo to search) without one, via an `include` list; `rick_workflow_control` acts via an `action` (pause/resume/cancel/retry/inject_guidance/approve_hint/reject_hint — reject's skip/fail rides on `reject_action` to avoid colliding with the discriminator); `rick_job_inspect` reads a job (status+output) or the global list/backends; `rick_workspace` (setup/cleanup/list) and `rick_confluence` (read/write) route via `action`; `rick_wave_manager` does plan/launch/status/cleanup/pr_links. `TestToolsList` asserts the exact count to guard against silent regrowth. Used by Claude Desktop/Cursor and the `agent/` Wails desktop UI. Full tool catalog: `internal/mcp/CLAUDE.md`. Agent UI architecture + slash commands: `agent/CLAUDE.md`.
...
| `RICK_MODEL` | Override default LLM model. Set by the agent UI to choose its model; flows into `backend.Request.Model`. When unset, the `claude` backend pins `claude-opus-4-8[1m]` (the 1M-context variant) in its driver — a headless `claude -p` otherwise defaults to the standard 200K-context model and overran the window on repos with heavy auto-loaded `.claude` context. The `antigravity` backend ignores it — `agy` has no model flag and picks the model from the logged-in Antigravity session. The `opencode` backend forwards it only when provider-qualified (`provider/model`, e.g. `google/gemini-2.5-pro`); a bare name is dropped and opencode uses its own configured default (a bare name like `gemini-2.5-flash` is rejected by opencode, so forwarding it would crash the call). |
