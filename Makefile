...
# rick-agent binary is NOT placed in ~/.local/bin — that path is reserved
# for `rick` (the server CLI). rick-agent only ships via the .deb into
# /usr/bin/rick-agent. The Wails build output stays under agent/build/bin/.
AGENT_BUILD    := $(AGENT_DIR)/build/bin/rick-agent
...
build-agent: ## Build rick-agent desktop app (Wails)
	...
	@rm -f $(HOME)/.local/bin/rick-agent
...
package: build-agent ## Build agent and create .deb package
	...
	cp $(AGENT_BUILD) /tmp/rick-agent-deb/usr/bin/rick-agent
