...
- `tools_wave.go` — Wave tools (4): `rick_wave_plan`, `rick_wave_launch`, `rick_wave_status`, `rick_wave_cleanup`. Topologically sorts children of a parent work item and batch-launches workflows. Accepts two source kinds via a `source` discriminator: `{type:"jira", epic:"PROJ-X"}` (back-compat shorthand `epic:"PROJ-X"`) or `{type:"github", parent:"owner/repo#N", child_discovery:"sub_issues"|"task_list"|"body_refs"|"auto", dependency_source:"table"|"body_refs"|"labels"|"none", dag_options:{dag_map:{"rick:foo":"dag-name","default":"workspace-dev"}}}`. GitHub path structured into discovery (sub_issues API, parent-body task-list, parent-body refs, or `auto` which tries all three), dependency derivation (parent-body Depends-on table, per-child body keywords, `depends:<N>` labels, or none), and per-child DAG selection (rick:* labels → dag_map, then open-PR cross-reference via issue timeline → `pr-feedback`, then `dag_map["default"]` or `workspace-dev`). Emits per-ticket `dag_params` so `rick_wave_launch` honors the plan's routing. Opt-in `RICK_GITHUB_GRAPHQL=1` env var collapses parent + sub-issues + timeline into a single GraphQL query; any error falls back to REST. Cleanup matches Jira children by key and GitHub children by `-gh<num>-` workspace suffix under `safeWorkspacePath`.
