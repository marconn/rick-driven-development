...
JSON-RPC 2.0 Model Context Protocol server exposing Rick's workflow, job, workspace, Jira, wave, observability, and Confluence capabilities to MCP clients (Claude Desktop, Cursor, the agent UI) over stdio and HTTP. 52 tools total.
...
- `tools_jobs.go` — Direct AI execution tools (7): `rick_consult`, `rick_run`, `rick_job_status`, `rick_job_output`, `rick_job_cancel`, `rick_jobs`, `rick_backends`. Backed by `JobManager` + `backend.Backend` subprocesses. No events. `rick_backends` reports the resolved default backend, the `RICK_REVIEW_BACKENDS` rotation, and per-backend whether the CLI binary is on PATH (`exec.LookPath`) — derived from `backend.Catalog` so it stays in sync with the factory.
