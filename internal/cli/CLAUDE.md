...
- `deps.go` — Shared dep constructors: `openEstimationStore`, `loadMicroserviceMap`, `newGitHubClient`, `openPluginStore`, `startOptionalServices` (GitHub reporter, CI poller, Jira poller — all gated by env vars), `parseBackendTimeout` (`RICK_BACKEND_TIMEOUT`), `newReviewBackend` (`RICK_REVIEW_BACKENDS` rotation with claude fallback on parse errors).
