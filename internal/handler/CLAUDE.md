...
- `ai.go` — `AIHandler` (base for all AI personas: loads context from event store, builds prompts via `persona.PromptBuilder`, calls backend, emits `AIRequestSent` + `AIResponseReceived`). `PlainText` config skips JSON extraction. **Session resume** (`resolveResume`, opt-in via `ResumeInFeedbackLoop` + `RICK_ENABLE_SESSION_RESUME`): on a feedback-driven re-run the handler sets `Request.SessionID` to the prior `AIResponsePayload.SessionID` and sends only the feedback delta (`buildResumePrompt`) instead of the full context prompt — the CLI session already holds task + codebase + prior work. Gated hard: same backend that opened the session, `autoRetryAttempt==0` (auto-retry rotates the backend, invalidating the session id — and gives a free full-context fallback), feedback present, non-RoundRobin backend. Wired for the developer only (handlers.go). Every run records its `Response.SessionID` into `AIResponseReceived` regardless of the flag.
...
- `context_snapshot.go` — non-AI; walks the workspace filesystem and git log to capture ground-truth codebase state (file tree, key files, schemas, recent commits) within size budgets. Feeds the developer prompt. **Backend-aware tree trim**: when the developer backend is `claude` AND the repo ships heavy auto-loaded Claude context (`hasHeavyClaudeContext`: root `CLAUDE.md` + `.claude/rules/*.md` ≥ 50KB), the file-tree dump (the snapshot's dominant cost) is omitted — claude reads `.claude/rules` and globs the tree itself. Key files + schema are always included, for every backend; gemini/codex/antigravity/opencode don't auto-load that context so they always get the tree. The developer backend name is captured from `Deps.Backend` at construction.
...
- `pr_stale_reference.go` — non-AI, opt-in (`RICK_ENABLE_STALE_REF_SWEEP`). Closes the cross-file gap of the `pr-docs-concordance` reviewer: reuses `generateWorkspaceDiff` to get the raw diff, extracts the exported symbols / `SCREAMING_SNAKE` consts / path-key literals the PR removed-without-re-adding (per-file removed−added is the precision core), `git grep`s the unchanged tracked files, and keeps hits only in doc files or code comments (executable-code hits are a compile/semantic concern other layers own). Emits `ContextEnrichment{Kind:"stale-references"}` (no event-type addition; bypasses diff-grounding since grep hits are ground truth). Caps symbols/findings and logs when truncating. Runs parallel to the reviewers; `pr-consolidator` folds it into an advisory, non-blocking section. Opt-in: injected into the DAG by `engine.WithStaleReferenceSweep` in `selectWorkflowDef` only when the env var is set.
...
| `default_optimistic` | LLM emitted no `VERDICT:` line **even after one reinforced retry**; `ParseVerdict` defaulted to PASS. | **Re-run that reviewer / suspect the backend.** `ReviewHandler.Handle` already re-prompts once (via `WithPromptReinforcement` + the `verdictReinforcement` instruction) when the first attempt is unparseable — so this value now means *both* attempts failed the format contract. The verdict carries zero signal; inspect `OutputRaw`. Persistent `default_optimistic` from one backend (historically antigravity) is a signal to drop it from `RICK_REVIEW_BACKENDS`. |
| `downgraded_no_grounded` | LLM emitted `VERDICT: FAIL` with N findings; all N were rejected by the diff-grounding filter. | Inspect `OutputRaw` to see the original findings, then check the matching `verdict.grounding.summary` for the `drop_reasons` breakdown. If `file_not_in_scope` dominates the LLM hallucinated; if `token_not_near_line` dominates the LLM cited valid lines but loose tokens — adjust the persona prompt or relax grounding. If `no_line_cited` dominates the LLM named a real in-scope file but omitted the `+`-line (and no token anchored a body-bullet rescue) — reinforce the "cite an exact `+` line" contract rule. |
