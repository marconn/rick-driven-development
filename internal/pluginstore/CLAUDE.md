...
Shared SQLite-backed state store for cross-plugin coordination — tracks processed Jira tickets and their workflow correlation IDs / PR linkage.
...
- Schema: a single `processed_tickets` table, ticket-keyed, with indexes on `correlation_id` and `status`. A legacy `ci_attempts` table may exist on older DBs; it is unused and harmless. It was dropped from migrations on 2026-04-23 together with the `github.CIPoller`.
- No namespacing per plugin and no TTL — schema is hardcoded for the Jira→workflow→PR pipeline, not a generic KV store despite the package name.
- Errors wrapped with `pluginstore:` prefix.
...
- `../github` — `fetcher.go` reads tickets via `GetTicketByCorrelation` as a PR-ref fallback when the workflow source lacks `gh:owner/repo#N`.
- `../cli` (`deps.go`) — wires `*Store` into the Jira poller at startup.
