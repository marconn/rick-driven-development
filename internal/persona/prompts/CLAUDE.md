...
- `pr-<category>.md` (14 files: `pr-security`, `pr-concurrency`, `pr-error-handling`, `pr-observability`, `pr-api-contract`, `pr-idempotency`, `pr-testing`, `pr-integration`, `pr-performance`, `pr-data`, `pr-hygiene`, `pr-vendor-resilience`, `pr-docs-concordance`, `pr-correctness`) — narrow single-concern PR reviewers with explicit `## Your Domain (ONLY these)` scope and cross-persona boundary rules. `pr-vendor-resilience` is polyglot (Go / JS-TS / PHP / network vendors) and self-scopes from the diff. `pr-docs-concordance` owns doc/comment-vs-code drift introduced by the diff (existing docs that now lie), distinct from `pr-hygiene` which owns *missing* docs and dead code. `pr-correctness` owns general behavioral correctness — logic bugs, broken invariants, regressions — deferring each specialized failure mode to its sibling; ambiguous-but-unprovable behavior is reported only as an advisory `minor`.
