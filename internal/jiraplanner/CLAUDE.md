...
- Idempotency: `createJiraIssues` is idempotent via external-truth dedup. It embeds a plain-text marker in the epic description (atomic with `CreateEpic`) and, before creating anything, runs a JQL search (`issuetype = Epic AND description ~ "<marker>"`) for an epic already carrying that marker. A hit short-circuits to a `ContextEnrichment` referencing the existing epic key — no duplicate epic/tasks on a boot RecoveryScan re-dispatch. The marker (`correlationMarker`) is a **single unbroken alphanumeric token** — `rickcorr` + the correlation ID with all non-alphanumerics stripped (e.g. `rickcorr<hexuuid>`) — because JQL `~` runs through Lucene, which tokenizes on `:`/`-`; a `rick-corr:<uuid>` form would split into separate terms and the search could miss a marker that is present. Both the embed and the lookup use `correlationMarker` so the form can't drift. Skipped when `correlationID` is empty (can't key the marker) or when the search errors (degrades to create rather than wedge the workflow). The two callers thread `env.CorrelationID`; `createJiraIssues` takes the minimal `jiraIssueWriter` interface (`Search`/`CreateEpic`/`CreateTask`/`LinkIssues`) so the path is stub-testable
