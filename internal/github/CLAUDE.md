...
- PR writes are NOT this package's job. The only writers are the `pr-reply-poster` and `pr-consolidator` handlers in `../handler` (see `../handler/CLAUDE.md`). Historical `Reporter` / `CIPoller` were removed on 2026-04-23 — their "post on every terminal event" model was a latent spam source once `JIRA_JQL` started populating tickets.
...
- `../pluginstore` — `Ticket` rows hold the `TicketID → repo + PR number + correlation` mapping used by the fetcher when the source lacks a PR ref.
