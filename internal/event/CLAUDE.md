...
- `VerdictGroundingSummaryPayload` — emitted once per `pr-category-review` invocation by `ReviewHandler`. Records `OriginalCount` (issues parsed from raw LLM output), `GroundedCount` (issues that survived the diff-anchoring filter), `RescuedCount` (subset of `GroundedCount` accepted via the file-scope rescue path — hallucinated line but token found elsewhere in the file's changed lines; issue survives with `Line=0`), `DropReasons` (`map[GroundingDropReason]int` over the four-value taxonomy: `file_not_in_scope`, `line_not_in_changed`, `token_not_near_line`, `rescued_file_scope` — the last is technically an accept, not a drop, but lives in the map for operator visibility), and the original/final `VerdictOutcome` so operators can spot demotions. Stored on the standard correlation aggregate alongside the matching `VerdictRendered`.
